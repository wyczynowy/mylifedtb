...
<html>
<head>
	<title>Elektronika - ogolne</title>
...
<body style="min-width: 1024px;">
	...
	<div class="row">
		<div class="horizontal-center" style="width: 22%; margin: 0 auto;">
			<h1><span class="label label-info">Elektronika - ogolne</span></h1>
		</div>
	</div>
		...
		<div class="col-sm-8">
			<div class="row">
				<security:authorize access="hasRole('ROLE_ADMIN') or hasRole('ROLE_SUPERADMIN')">
					<div class="col-lg-3"><button class="btn btn-success" onclick="window.location.href='${pageContext.request.contextPath}/electronicsgeneral/addnote'">Dodaj notatke +</button></div>
				</security:authorize>
			</div>
			<br/>
			<div class="row">				
				<div class="panel-group" id="accordion">		
					<c:forEach var="advice" items="${electronicsAdvices}">
						<div class="panel panel-default">
							<div class="panel-heading">
								<div class="panel-title">
									<security:authorize access="hasRole('ROLE_SUPERADMIN')">
										<button class="btn btn-danger btn-xs" onclick="window.location.href='${pageContext.request.contextPath}/electronicsgeneral/deleteNote?adviceId=${advice.id}'">Usun</button>
									</security:authorize>
									<security:authorize access="hasRole('ROLE_SUPERADMIN') or hasRole('ROLE_ADMIN')">
										<button class="btn btn-warning btn-xs" onclick="window.location.href='${pageContext.request.contextPath}/electronicsgeneral/editNote?adviceId=${advice.id}'">Edytuj</button>				
									</security:authorize>
							...
							<div id="${advice.id}" class="panel-collapse collapse">
								<div class="panel-body">
									...
									<br>Załączniki:<br>
									<c:forEach var="attachment" items="${electronicsAdvicesAttachments}">
										<c:if test="${attachment.electronicsGeneralAdviceId == advice.id}">
